...
- 支持经清洗的 GFM、代码高亮与复制；自动滚动根据内容实际布局变化持续跟随。用户操作导致视口向上且底部距离增加时立即暂停，但内容收缩造成的滚动位置钳制仍保持跟随；两种情况都不会停止正在进行的流式输出。
...
- Vitest：shared 8 项、API 20 项、Web 18 项，共 46 项单元与集成测试通过。
- Playwright：渐进输出与刷新恢复、思考过程展示与恢复、停止并保留部分内容、规范化错误态、流式代码块自动跟随、思考折叠后的最终回答跟随、用户上滚暂停跟随和移动端无横向溢出，共 10 项通过。
...
- 思考面板自动折叠导致最终回答停止跟随的问题已完成修复、10 次重复回归和用户验收；主对话现以最大滚动位置及底部距离区分布局收缩和用户上滚，详见 [`doc/fixes/reasoning_panel_collapse_auto_scroll.md`](../../doc/fixes/reasoning_panel_collapse_auto_scroll.md)。
...
- 2026-08-08：修复思考面板自动折叠时内容收缩被误判为用户上滚的问题；新增阶段切换回归测试并经用户本地验收。
