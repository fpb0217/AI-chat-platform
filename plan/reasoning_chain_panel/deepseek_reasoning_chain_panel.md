...
- 思考面板在 answer 阶段自动折叠时，主对话滚动需要区分“内容收缩导致浏览器钳制 `scrollTop`”与“用户主动上滚”，前者继续跟随最终回答，后者立即暂停。
...
- 思考面板自动折叠后，长最终回答仍持续贴近主对话底部；布局收缩不能误关自动跟随。
...
- 功能验收后的真实使用又发现 answer 阶段自动折叠会缩短主对话内容；浏览器随之向上钳制 `scrollTop`，旧逻辑将其误判为用户上滚并关闭最终回答跟随。修复记录见 [`doc/fixes/reasoning_panel_collapse_auto_scroll.md`](../../doc/fixes/reasoning_panel_collapse_auto_scroll.md)。
- 主对话现同时记录 `scrollTop`、最大滚动位置和底部距离；最大滚动位置缩小且底部距离未增加时视为布局调整，只有底部距离实际增加才视为用户上滚。
...
- Playwright：流式回答、思考展开/折叠/刷新恢复、思考阶段停止、长思考内部滚动、思考折叠后的最终回答跟随、答案阶段停止、错误、自动滚动、用户阅读位置和移动端，共 10 项通过；新增折叠切换用例重复 10 次全部通过。
...
- 2026-08-08：用户反馈并稳定复现思考面板自动折叠后最终回答停止跟随；修复布局收缩被误判为用户上滚的问题，补充 Playwright 回归及 10 次重复验证，用户本地测试通过并验收。
